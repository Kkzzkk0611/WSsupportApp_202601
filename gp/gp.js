...
(() => {
  ...
  function buildCarousel(items) {
    ...
    if (!carouselTrack.dataset.initialized) {
      ...
      if (topCard) {
        ...
        const targetScrollTop = cardTop + cardHeight / 2 - containerHeight / 2;
  ...
  function openArtDialog(a) {
    ...
    // ▼▼▼ [修正] ダイアログのUI要素を取得 ▼▼▼
    const dialogBody = artDialog.querySelector(".art-dialog-body");
    const dialogFooter = artDialog.querySelector(".art-dialog-actions");

    // ▼▼▼ [追加] トップカードの場合の特別処理 ▼▼▼
    if (a.art_id === "art_top") {
      artDialogTitle.textContent = "ご近所アートチャットとは？";

      // ユーザー指定のHTMLを挿入
      dialogBody.innerHTML = `
        <video
          src="../top/images/chat.mp4" 
          autoplay
          muted
          loop
          playsinline
          class="demo-video" 
        ></video>
        <p class="top-card-intro-text">
          「ご近所アートチャット」は、地域の人たちが制作した防災アート作品を見ながら、感想や気づきを共有できるページです。<br>忙しい毎日の合間でも、作品を眺めて「いいね」と思ったことを言葉にするだけ。
          <br>それが、地域の防災やまちづくりを考える小さな一歩になります。
          <br>作品を通じて、同じ地域に暮らす人の想いや発見を知ることで、
          <br>自分のまちへの関心や安心感が少しずつ広がっていきます。
          <br>コメントやリアクションを通じて、地域住民の防災意識を育みます。
        </p>
      `;
      ...
      artDialog.showModal?.();
      return; // 通常の作品表示処理をスキップ
    }
    // ▲▲▲ [追加] ここまで ▲▲▲

    // ▼▼▼ [追加] 通常カードの場合はフッターを再表示（念のため） ▼▼▼
  ...
  `;
    ...
    dialogBody.innerHTML = contentHtml; // [修正] 変数を使用
    artDialog.showModal?.();
